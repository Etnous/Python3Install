...
--Please choose python version--
  ${Green_font_prefix}1.${Font_color_suffix}  3.7.2
  ${Green_font_prefix}2.${Font_color_suffix}  3.7.3
  ${Green_font_prefix}3.${Font_color_suffix}  3.7.4
  ${Green_font_prefix}4.${Font_color_suffix}  3.7.5
---------------------------------
"
read -e -p "Please input the num[1-4]:" ver_num
case $ver_num in
  1)
    py_v="3.7.2" && select_area && install_python
  ;;
  2)
    py_v="3.7.3" && select_area && install_python
  ;;
  3)
    py_v="3.7.4" && select_area && install_python
  ;;
  4)
    py_v="3.7.5" && select_area && install_python
  ;;
  *)
    echo -e "${error}Please input the right num!" && exit 1
  ;;
...
check_python_status(){
  if [[ $os_version == "ubuntu" ]]; then
      ...
      if [ $? -ne 0 ]; then
        echo -e "Python Version: ${Red_font_prefix}Not installed${Font_color_suffix}"
        rm -rf python_version
      else
        py_ver=$(cat python_version)
        echo -e "Python Version: ${Green_font_prefix}$py_ver${Font_color_suffix}"
        rm -rf python_version
      fi
  else
...
--Python Installation and Management Script--
 ...
 ${Green_font_prefix}2.${Font_color_suffix}  Install Basic Web Scraping Frameworks
---------------------------------------------
"
check_python_status
