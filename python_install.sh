...
case $ver_num in
  1)
    py_v="3.6.9" && select_area && install_python
  ;;
  2)
...
check_python_status(){
  ...
  if [[ ! $py_ver ]]; then
      ...
      if [ $? -ne 0 ]; then
        echo -e "Default Python Version: ${Red_font_prefix}Not installed${Font_color_suffix}"
        rm -rf python_version
      else
        py_ver=$(cat python_version)
        echo -e "Default Python Version: ${Green_font_prefix}$py_ver${Font_color_suffix}"
        rm -rf python_version
      fi
  else
      echo -e "Default Python Version: ${Green_font_prefix}$py_ver${Font_color_suffix}"
